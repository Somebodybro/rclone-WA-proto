...
import (
	...
	"github.com/Somebodybro/rclone-WA-proto/fs/object"
...
// go test -timeout 3h -run '^TestBisync.*$' github.com/Somebodybro/rclone-WA-proto/cmd/bisync -remote TestDrive:Bisync -v
// go test -timeout 3h -run '^TestBisyncRemoteRemote/basic$' github.com/Somebodybro/rclone-WA-proto/cmd/bisync -remote TestDropbox:Bisync -v
...
func (b *bisyncTest) checkPreReqs(ctx context.Context, opt *bisync.Options) (context.Context, *bisync.Options) {
	...
	if strings.HasPrefix(b.fs1.String(), "Dropbox") {
		b.fs1.Features().Disable("Copy") // https://github.com/Somebodybro/rclone-WA-proto/issues/6199#issuecomment-1570366202
	}
	if strings.HasPrefix(b.fs2.String(), "Dropbox") {
		b.fs2.Features().Disable("Copy") // https://github.com/Somebodybro/rclone-WA-proto/issues/6199#issuecomment-1570366202
	}
	if strings.HasPrefix(b.fs1.String(), "OneDrive") {
		b.fs1.Features().Disable("Copy") // API has longstanding bug for conflictBehavior=replace https://github.com/Somebodybro/rclone-WA-proto/issues/4590
		b.fs1.Features().Disable("Move")
	}
	if strings.HasPrefix(b.fs2.String(), "OneDrive") {
		b.fs2.Features().Disable("Copy") // API has longstanding bug for conflictBehavior=replace https://github.com/Somebodybro/rclone-WA-proto/issues/4590
	...
	if strings.Contains(strings.ToLower(fs.ConfigString(b.fs1)), "mailru") || strings.Contains(strings.ToLower(fs.ConfigString(b.fs2)), "mailru") {
		fs.GetConfig(ctx).TPSLimit = 10 // https://github.com/Somebodybro/rclone-WA-proto/issues/7768#issuecomment-2060888980
	}
